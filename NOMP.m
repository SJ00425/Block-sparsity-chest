...
K_max = 3;          % Max # of paths to extract
...
ov_phi = 2;         % AoA grid oversampling (~ Nrx*ov_phi points)
...
Nphi = Nrx * ov_phi;
u = linspace(-1, 1, Nphi);
phi_grid = asin(max(min(u,1),-1));  % [-pi/2, pi/2]
A_phi = exp(-1j*2*pi*(d/lambda) * n_vec * sin(phi_grid));   % (Nrx x Nphi)
...
for k = 1:K_max
    ...
    [phi_r, tau_r, nu_r, alpha_r, R] = refine_atom_newton( ...
        R, phi0, tau0, nu0, ...
        f0, Tsym, d/lambda, m_idx, s_idx, ...
...
[Gamma_ls, R_final] = debias_joint_ls(R + 0, Y, est, Nrx, M, Ns, f0, Tsym, d/lambda, n_vec, m_idx, s_idx);
est.gamma = Gamma_ls;
est.R     = R_final;

%% ========= REPORT ===========================================
T = table((1:numel(est.phi)).', est.phi, est.tau, est.nu, est.gamma, ...
    'VariableNames', {'path','phi_rad','tau_s','nu_Hz','gamma'});
disp(T);

H_est = reconstruct_H_from_est(est, Nrx, M, Ns, f0, Tsym, d, lambda, g_m);
fprintf('NMSE = %.2f dB\n', nmse_db(H, H_est));


% opts = struct('tau_unit','us','title','DD scatter (no pairing)');
% plot_delay_doppler_scatter(true_tau, true_nu, gamma, ...
%                            est.tau,   est.nu,  est.gamma, opts);
